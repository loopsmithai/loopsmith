...
- **authority** — `observe` (read the stream) · `inject` (send input/prompts into the session) · `approve` (answer the session's permission requests). These three verbs are exactly the ones the first-party proposals for multi-client session attach converged on, which validates the dial ([R-04](research/R-04-agent-stream-acp.md) §5).
...
**Note on network attach.** The rich stream is OOTB, but multi-surface **network** attach to a running ephemeral session is *not* shipped by any agent CLI today (the `claude serve` and MCP-session-channel proposals are both unshipped — [R-04](research/R-04-agent-stream-acp.md) §1). So the relay's network fan-out is genuinely ours to build; if either proposal ships, the relay sits *on top of* it rather than being obsoleted. A comparable production system (agent-control-plane) independently built exactly this — a DB-backed session with `Watch`/`Push` streaming for multi-client fan-out and injection ([R-05](research/R-05-agent-control-plane.md)) — which is corroboration that this component is real and necessary, and a candidate reference for its construction.
